...
//   return null;
// }
const BASE_URL = 'https://helpful-on-corgi.ngrok-free.app';
...
async function Viewer() {
  try {
    ...
    const response = await fetch(`${BASE_URL}/api/v1/device/viewer`, {
      method: 'POST',
      headers: {
        'accept': '*/*',
      },
    ...
    const encryptedData = data.data.encrypt;

    const decryptRes = await fetch(`${BASE_URL}/api/v1/decode`, {
      method: 'POST',
      headers: {
        'accept': '*/*',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ code: 200, data: encryptedData })
    });

    if (!decryptRes.ok) throw new Error(`HTTP error! status: ${decryptRes.status}`);

    const decrypted = await decryptRes.json();
    const targetValue = parseInt(decrypted.data.data);
    const currentValue = parseInt(viewer.innerText || "0");

    animateCount(viewer, currentValue, targetValue, 2000);
  } catch (error) {
